...
$result = @{cpu_ghz=$null; cpu_temp=$null; gpu=$null; gpu_temp=$null}
try {
...
foreach ($namespace in @('root\LibreHardwareMonitor', 'root\OpenHardwareMonitor')) {
    try {
        ...
        if ($sensors.Count) { $result.cpu_temp = [math]::Round(($sensors | Measure-Object Value -Maximum).Maximum); break }
    } catch {}
}
    ...
    def _run(self):
        ...
        while not self.stop_event.is_set():
            ...
            try:
                ...
                pass
            try:
                env = dict(os.environ, K1001_GPU_FALLBACK='0' if 'gpu' in values else '1')
